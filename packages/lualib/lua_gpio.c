...
static int lgpio_mode( lua_State* L )
{
 ...
 // unsigned pullup = luaL_optinteger( L, 3, FLOAT );
  ...
  luaL_argcheck(L, platform_gpio_exists(pin) && ( pin>0), 1, "Invalid pin");
...
LUA_LIB_DEBUG("pin=%d,mode=%d,pullup=%d\n",pin, mode);
  ...
  if( platform_gpio_mode( pin, mode) < 0 )
...
{
	unsigned pin = luaL_checkinteger( L, 1 );
	platform_gpio_exists(pin);
	int level=platform_gpio_read( pin );
	LUA_LIB_DEBUG("read_pin=%d,read_value=%d\n", pin, level);
...
static int lgpio_write( lua_State* L )
{
	...
	LUA_LIB_DEBUG("write_pin=%d,write_value=%d\n", pin, level);
	platform_gpio_write(pin, level);
	return 0;
}

#define DELAY_TABLE_MAX_LEN 256
#define delayMicroseconds rt_thread_delay
...
static int lgpio_serout( lua_State* L )
{
  ...
  rt_uint32_t *delay_table = (rt_uint32_t *)rt_malloc(table_len);

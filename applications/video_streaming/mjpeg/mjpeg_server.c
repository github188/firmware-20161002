...
void mjpeg_server_thread(void *arg)
{
    ...
    while (!server->stop) {
        if (client->fd == -1) {
            ...
            client->fd = accept(server->fd, (struct sockaddr *)&client->addr, &sock_len);
            if (client->fd < 0)
                continue;
...
int mjpeg_start_server(int port)
{
    ...
    g_server.thread_id = rt_thread_create("mjpeg_server",
                                          mjpeg_server_thread,
                                          &g_server,
                                          1024,
                                          17,
